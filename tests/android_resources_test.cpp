...
TEST(AndroidResourceErrorTest, ErrorToString) {
  ...
  EXPECT_EQ(to_string(AndroidResourceError::InvalidResourceFile), "Invalid resource file");
  EXPECT_EQ(to_string(AndroidResourceError::CompilationFailed), "Resource compilation failed");
  EXPECT_EQ(to_string(AndroidResourceError::LinkingFailed), "Resource linking failed");
}
...
TEST_F(ResourceTestFixture, ResourceFileComputeHashDifferentContent) {
  create_test_resource("res/values/strings1.xml",
                       "<resources><string name=\"a\">A</string></resources>");
  create_test_resource("res/values/strings2.xml",
                       "<resources><string name=\"b\">B</string></resources>");
...
TEST_F(ResourceTestFixture, IncrementalSaveAndLoadState) {
  ...
  // Save state
  bool saved = resource_incremental::save_compilation_state(state_file, config, compilation_hash);
  EXPECT_TRUE(saved);
