...
public:
  void add_task(MockTask task) { queue_.push(std::move(task)); }
  ...
  auto size() const -> size_t { return queue_.size(); }
...
// Generate mock tasks
auto generate_mock_tasks(int num_tasks, int max_priority)
    -> std::vector<MockTask> {
...
BENCHMARK(BM_TaskScheduling_AddTasks)
    ->RangeMultiplier(10)
    ->Range(10, 10000)
    ->Complexity();

BENCHMARK(BM_TaskScheduling_GetNextTask)
    ->RangeMultiplier(10)
    ->Range(10, 10000)
    ->Complexity();

BENCHMARK(BM_TaskScheduling_FullCycle)
    ->RangeMultiplier(10)
    ->Range(10, 10000)
    ->Complexity();

BENCHMARK(BM_TaskScheduling_VariablePriority)
    ->Arg(10)
    ->Arg(100)
    ->Arg(1000)
    ->Arg(10000);

BENCHMARK(BM_TaskScheduling_Incremental)
    ->RangeMultiplier(10)
    ->Range(10, 10000)
    ->Complexity();
