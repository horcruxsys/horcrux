...
auto AndroidToolchainDetector::detect() 
    -> tl::expected<AndroidToolchain, AndroidToolchainError> {
  // Detect SDK root
...
auto AndroidToolchainDetector::validate(const AndroidToolchain& toolchain) 
  ...
  if (toolchain.ndk_root && 
      !AndroidToolchainValidator::validate_ndk(*toolchain.ndk_root)) {
...
auto AndroidToolchainDetector::detect_sdk_root() 
...
auto AndroidToolchainDetector::detect_java_home() 
    -> std::optional<std::filesystem::path> {
  // Try JAVA_HOME
...
auto AndroidToolchainDetector::scan_build_tools(const std::filesystem::path& sdk_root)
  ...
  std::sort(build_tools.begin(), build_tools.end(),
           [](const AndroidBuildTools& a, const AndroidBuildTools& b) {
...
auto AndroidToolchainDetector::scan_ndks(const std::filesystem::path& ndk_root)
  ...
  std::sort(ndks.begin(), ndks.end(),
           [](const AndroidNdk& a, const AndroidNdk& b) {
             return a.version > b.version;
           });
    ...
    while (std::getline(release, line)) {
      if (line.find("JAVA_VERSION") != std::string::npos) {
        ...
        if (pos != std::string::npos) {
          ...
          // Remove quotes
          java_sdk.version.erase(
              std::remove(java_sdk.version.begin(), java_sdk.version.end(), '"'),
