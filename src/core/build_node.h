...
public:
    ...
    [[nodiscard]] auto label() const -> const Label& { return label_; }

    /// @brief Get the type of this node
    /// @return The node type string
    [[nodiscard]] auto node_type() const -> const NodeType& { return node_type_; }

    /// @brief Get the input files/resources
    /// @return Vector of input paths
    [[nodiscard]] auto inputs() const -> const std::vector<std::string>& { return inputs_; }

    /// @brief Get the expected output files
    /// @return Vector of output paths
    [[nodiscard]] auto outputs() const -> const std::vector<std::string>& { return outputs_; }
