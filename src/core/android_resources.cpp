...
auto ResourceQualifiers::to_string() const -> std::string {
  ...
  if (locale) ss << *locale << "-";
  if (region) ss << "r" << *region << "-";
  if (density) ss << resource_utils::density_to_string(*density) << "-";
  if (screen_size) ss << *screen_size << "-";
  if (orientation) ss << *orientation << "-";
  if (night_mode) ss << *night_mode << "-";
  if (api_level) ss << "v" << *api_level;
...
auto ResourceFile::get_type(const std::filesystem::path& path) -> ResourceType {
  if (!path.has_parent_path()) return ResourceType::Unknown;
  ...
  if (base_type == "values") return ResourceType::Values;
  if (base_type == "layout") return ResourceType::Layout;
  if (base_type == "drawable") return ResourceType::Drawable;
  if (base_type == "mipmap") return ResourceType::Mipmap;
  if (base_type == "raw") return ResourceType::Raw;
  if (base_type == "xml") return ResourceType::Xml;
  if (base_type == "anim") return ResourceType::Anim;
  if (base_type == "animator") return ResourceType::Animator;
  if (base_type == "color") return ResourceType::Color;
  if (base_type == "menu") return ResourceType::Menu;
...
auto AndroidResourceProcessor::get_aapt2_path() const 
  -> std::optional<std::filesystem::path> {
  return aapt2_path_;
}

auto AndroidResourceProcessor::find_aapt2() const 
  -> std::optional<std::filesystem::path> {
  // Search for AAPT2 in build tools (prefer latest version)
  for (auto it = toolchain_.build_tools.rbegin(); 
       it != toolchain_.build_tools.rend(); ++it) {
...
auto AndroidResourceProcessor::link(const Aapt2LinkConfig& config)
  ...
  result.link_time = std::chrono::duration_cast<std::chrono::milliseconds>(
    end_time - start_time);
...
auto AndroidResourceProcessor::merge_resources(const ResourceMergeConfig& config)
  ...
  for (const auto& res_dir : config.resource_dirs) {
    ...
    all_resources.insert(all_resources.end(),
                        scan_result->begin(), scan_result->end());
  }
  ...
  for (const auto& resource : all_resources) {
    auto relative_path = std::filesystem::relative(resource.path, 
                                                   resource.path.parent_path().parent_path());
  ...
  auto end_time = std::chrono::steady_clock::now();
  result.merge_time = std::chrono::duration_cast<std::chrono::milliseconds>(
    end_time - start_time);
  ...
  for (const auto& resource : result.merged_resources) {
    SHA256_Update(&sha256_ctx, resource.content_hash.c_str(), 
                 resource.content_hash.length());
  }
...
auto AndroidResourceProcessor::process_pipeline(const PipelineConfig& config)
  ...
  SHA256_Update(&sha256_ctx, merge_result->merge_hash.c_str(), 
               merge_result->merge_hash.length());
  SHA256_Update(&sha256_ctx, compile_result->compilation_hash.c_str(),
               compile_result->compilation_hash.length());
  SHA256_Update(&sha256_ctx, link_result->link_hash.c_str(),
               link_result->link_hash.length());
...
auto AndroidResourceProcessor::validate_compile_config(
...
auto AndroidResourceProcessor::compute_compile_hash(
  const Aapt2CompileConfig& config) -> std::string {
...
auto AndroidResourceProcessor::compute_link_hash(const Aapt2LinkConfig& config)
  -> std::string {
  ...
  if (manifest_hash_result) {
    SHA256_Update(&sha256_ctx, manifest_hash_result->c_str(),
                 manifest_hash_result->length());
  }
...
auto resource_type_to_string(ResourceType type) -> std::string {
  switch (type) {
    case ResourceType::Values: return "values";
    ...
    case ResourceType::Xml: return "xml";
    case ResourceType::Anim: return "anim";
    case ResourceType::Animator: return "animator";
...
auto string_to_resource_type(const std::string& type_str) -> ResourceType {
  if (type_str == "values") return ResourceType::Values;
  if (type_str == "layout") return ResourceType::Layout;
  if (type_str == "drawable") return ResourceType::Drawable;
  if (type_str == "mipmap") return ResourceType::Mipmap;
  if (type_str == "raw") return ResourceType::Raw;
  if (type_str == "xml") return ResourceType::Xml;
  if (type_str == "anim") return ResourceType::Anim;
  if (type_str == "animator") return ResourceType::Animator;
  if (type_str == "color") return ResourceType::Color;
  if (type_str == "menu") return ResourceType::Menu;
  return ResourceType::Unknown;
}

auto density_to_string(ResourceDensity density) -> std::string {
  switch (density) {
    case ResourceDensity::LDPI: return "ldpi";
    case ResourceDensity::MDPI: return "mdpi";
    case ResourceDensity::HDPI: return "hdpi";
    case ResourceDensity::XHDPI: return "xhdpi";
    case ResourceDensity::XXHDPI: return "xxhdpi";
    case ResourceDensity::XXXHDPI: return "xxxhdpi";
    case ResourceDensity::NODPI: return "nodpi";
    case ResourceDensity::TVDPI: return "tvdpi";
    case ResourceDensity::ANYDPI: return "anydpi";
    case ResourceDensity::None: return "";
  }
  return "";
}

auto string_to_density(const std::string& density_str) -> ResourceDensity {
  if (density_str == "ldpi") return ResourceDensity::LDPI;
  if (density_str == "mdpi") return ResourceDensity::MDPI;
  if (density_str == "hdpi") return ResourceDensity::HDPI;
  if (density_str == "xhdpi") return ResourceDensity::XHDPI;
  if (density_str == "xxhdpi") return ResourceDensity::XXHDPI;
  if (density_str == "xxxhdpi") return ResourceDensity::XXXHDPI;
  if (density_str == "nodpi") return ResourceDensity::NODPI;
  if (density_str == "tvdpi") return ResourceDensity::TVDPI;
  if (density_str == "anydpi") return ResourceDensity::ANYDPI;
...
auto save_compilation_state(const std::filesystem::path& state_file,
  ...
  for (const auto& resource : config.resources) {
    file << "resource=" << resource.path.string() 
